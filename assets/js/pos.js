  ...
  function initStorage() {
    try {
      ...
      try {
        ...
        console.warn('Store initialization failed, trying alternative method...');
        ...
        storage = {
          ...
          set: function (key, value) {
            try {
              localStorage.setItem('electron-store_' + key, JSON.stringify(value));
            } catch (e) {
  ...
  function initializeApp() {
    ...
    function loadProducts() {
      $.get(api + 'inventory/products', function (data) {
        ...
        if (filterInstituteId > 0) {
          ...
          displayProducts = data.filter((product) => product.institute_id == filterInstituteId);

          // Filter categories that belong to the selected institute
          displayCategories = allCategories.filter((cat) => cat.institute_id == filterInstituteId);
                            ...
                            <div class="widget-panel widget-style-2 ">                    
                                        ...
                                        <div style="margin-top: 10px;">
                                          ...
                                          <span class="count" style="font-size: 13px; font-weight: 500; color: ${
                                            item.stock == 1 && item.quantity <= 5 ? '#f56954' : '#00a65a'
                                          }">${item.stock == 1 ? item.quantity : 'N/A'}</span>
                                        </div>
                                        </div>
                                         <sp class="text-success text-center" style="display: block; margin-top: 12px; font-size: 18px;"><b data-plugin="counterup">${
                                           (settings && settings.symbol ? settings.symbol : '$') + item.price
                                         }</b> </sp>
                            </div>
                        </div>`;
    ...
    function populateProductCategories(instituteId) {
      ...
      if (instituteId && instituteId != '') {
        filteredCategories = allCategories.filter((cat) => cat.institute_id == instituteId);
    ...
    function loadInstitutes() {
      $.get(api + 'institutes/all', function (data) {
        ...
        $('#filter_institute').html(`<option value="0">All Institutes</option>`);
        $('#filter_institute_products').html(`<option value="0">All Institutes</option>`);
        data.forEach((institute) => {
          $('#filter_institute').append(`<option value="${institute.id}">${institute.name}</option>`);
          $('#filter_institute_products').append(`<option value="${institute.id}">${institute.name}</option>`);
        });

        // Populate category institute dropdown
        $('#category_institute').html(`<option value="">Select Institute</option>`);
        data.forEach((institute) => {
          $('#category_institute').append(`<option value="${institute.id}">${institute.name}</option>`);
        });
      });
    }
    ...
    function loadProductsView() {
      let currentFilterInstitute = parseInt($('#filter_institute_products').val()) || 0;

      let displayProducts = allProducts;
      let displayCategories = allCategories;

      if (currentFilterInstitute > 0) {
        displayProducts = allProducts.filter((product) => product.institute_id == currentFilterInstitute);
        displayCategories = allCategories.filter((cat) => cat.institute_id == currentFilterInstitute);
      }
    ...
    $.fn.editInstitute = function (index) {
      ...
      if (user.role === 'cashier' || user.role === 'user') {
        Swal.fire('Access Denied', 'Cashiers cannot edit institutes', 'warning');
        return;
      }
    ...
    $.fn.deleteInstitute = function (id) {
      ...
      if (user.role === 'cashier' || user.role === 'user') {
        Swal.fire('Access Denied', 'Cashiers cannot delete institutes', 'warning');
        return;
      }
      Swal.fire({
        title: 'Are you sure?',
    ...
    $('#skuCode').on('input', function () {
      ...
      if (matchingProducts.length > 0) {
        ...
        $('#searchResults').html("<div style='padding: 10px; color: #999;'>No products found</div>").show();
      }
    });
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && data._id != undefined && data.quantity >= 1) {
            ...
            Swal.fire('Out of stock!', 'This item is currently unavailable', 'info');
            $('#basic-addon2').empty();
            $('#basic-addon2').append($('<i>', { class: 'glyphicon glyphicon-remove' }));
          } else {
        ...
        error: function (xhr, status, error) {
          ...
          Swal.fire('Error', 'Could not add product. Please try again.', 'error');
          $('#basic-addon2').empty();
          $('#basic-addon2').append($('<i>', { class: 'glyphicon glyphicon-warning-sign' }));
        },
      });
    }
      ...
      $.ajax({
        ...
        success: function (data) {
          if (data && data._id != undefined && data.quantity >= 1) {
            ...
            Swal.fire('Not Found!', 'Product with ID <b>' + searchValue + '</b> not found!', 'warning');
        ...
        error: function (xhr, status, error) {
          ...
          Swal.fire('Error', 'Could not search for product. Please try again.', 'error');
          $('#basic-addon2').empty();
          $('#basic-addon2').append($('<i>', { class: 'glyphicon glyphicon-warning-sign' }));
        },
      });
    ...
    $.fn.calculateCart = function () {
      ...
      $('#price').text((settings && settings.symbol ? settings.symbol : '$') + total.toFixed(2));
      ...
      $('#gross_price').text((settings && settings.symbol ? settings.symbol : '$') + grossTotal.toFixed(2));
    ...
    $.fn.calculateChange = function () {
      ...
      $('#change_amount').text(
        (settings && settings.symbol ? settings.symbol : '$') + (change > 0 ? change.toFixed(2) : '0.00'),
      );
      ...
      $.each(cartList, function (index, data) {
        $('#cartTable > tbody').append(
          $('<tr>').append(
            ...
            $('<td>', {
              text: (settings && settings.symbol ? settings.symbol : '$') + (data.price * data.quantity).toFixed(2),
            }),
            $('<td>').append(
              $('<button>', {
    ...
    function generateAndShowInvoice() {
      ...
      let companyName = settings && settings.company_name ? settings.company_name : 'Creative Hands By Tevta';
      ...
      let companyFooter = settings && settings.footer ? settings.footer : 'Thank you for your business!';
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          cartCopy.forEach((item) => {
            ...
            items += `
              <tr style="border-bottom: 1px solid #eee;">
                ...
                <td style="padding: 12px 8px; text-align: right;">${currency}${itemTotal.toFixed(2)}</td>
              </tr>`;
          });
          ...
          let receipt = `<div style="max-width: 800px; margin: 0 auto; padding: 40px; font-family: Arial, sans-serif; background: #fff;">                            
            ...
            <div style="padding: 30px; border-radius: 8px; margin-bottom: 30px;">
                ...
                <p style="margin: 5px 0 0 0; font-size: 12px; color: #666;">${companyAddress || ''}</p>
                <p style="margin: 0; font-size: 12px; color: #666;">${companyContact || ''}</p>
            </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
            ...
            <div style="text-align: right;">
                ...
                <div>
                    ...
                    <p style="margin: 0; font-size: 14px; color: #333;">${user.fullname || 'Administrator'}</p>
                </div>
            </div>
        </div>
            ...
            <tbody>
                ${cart
                  .map(
                    (item) => `
                <tr style="border-bottom: 1px solid #eee;">
                    <td style="padding: 12px 8px; font-size: 12px; color: #333;">${item.product_name}</td>
                ...
                <tr>
                    ...
                    <td style="text-align: right; padding: 8px; font-size: 12px;">${parseFloat(discount).toFixed(
                      2,
                    )}</td>
                </tr>
                `
                ...
                <tr>
                    ...
                    <td style="text-align: right; padding: 8px; font-size: 12px;">${parseFloat(totalVat).toFixed(
                      2,
                    )}</td>
                </tr>
                `
        ...
        <div style="display: flex; justify-content: space-between; padding: 20px; border-radius: 8px;">
            ...
            <div style="text-align: right;">
                ...
                <p style="margin: 0; font-size: 32px; font-weight: 700; color: #000;">${parseFloat(orderTotal).toFixed(
                  2,
                )}</p>
            </div>
        </div>
      ...
      receipt = `<div style="max-width: 800px; margin: 0 auto; padding: 40px; font-family: Arial, sans-serif; background: #fff;">                            
        ...
        <div style="background: linear-gradient(to right, #f9f3e8, #f5e6d3); padding: 30px; border-radius: 8px; margin-bottom: 30px;">
            ...
            <div style="text-align: right; margin-bottom: 20px;">
                <h2 style="margin: 0; font-size: 16px; color: #333; font-weight: 600;">${settings.store}</h2>
                ...
                <p style="margin: 0; font-size: 12px; color: #666;">${settings.tax ? 'Tax ID: ' + settings.tax : ''}</p>
            </div>
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
            ...
            <div style="text-align: right;">
                ...
                <div>
                    <p style="margin: 0; font-weight: 700; font-size: 12px; color: #000;">Cashier</p>
                    <p style="margin: 0; font-size: 14px; color: #333;">${user.fullname}</p>
                </div>
            </div>
        </div>
        ...
        <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
            ...
            <tbody>
                ...
                <tr>
                    <td colspan="3" style="text-align: right; padding: 8px; font-size: 12px;">Discount:</td>
                    <td style="text-align: right; padding: 8px; font-size: 12px;">${settings.symbol}${parseFloat(
                        discount,
                      ).toFixed(2)}</td>
                </tr>
                `
        ...
        <div style="display: flex; justify-content: space-between; background: linear-gradient(to right, #f9f3e8, #fde8b8); padding: 20px; border-radius: 8px;">
            <div style="flex: 1; padding-right: 20px;">
                ...
                ${refNumber ? `<p style="margin: 5px 0 0 0; font-size: 11px; color: #888;">Ref: ${refNumber}</p>` : ''}
            </div>
            <div style="text-align: right;">
                <p style="margin: 0 0 10px 0; font-size: 14px; font-weight: 700; color: #000;">TOTAL</p>
                <p style="margin: 0; font-size: 32px; font-weight: 700; color: #000;">${settings.symbol}${parseFloat(
        orderTotal,
      ).toFixed(2)}</p>
            </div>
        </div>
    ...
    $('#newProductModal').click(function () {
      ...
      let instituteOptions = '<option value="">Select Institute (Optional)</option>';
    ...
    $.fn.editCategory = function (index) {
      ...
      if (user.role === 'cashier' || user.role === 'user') {
        Swal.fire('Access Denied', 'Cashiers cannot edit categories', 'warning');
        return;
      }
    ...
    $.fn.deleteProduct = function (id) {
      ...
      if (user.role === 'cashier' || user.role === 'user') {
        Swal.fire('Access Denied', 'Cashiers cannot delete products', 'warning');
        return;
      }
      Swal.fire({
        title: 'Are you sure?',
    ...
    $.fn.deleteCategory = function (id) {
      ...
      if (user.role === 'cashier' || user.role === 'user') {
        Swal.fire('Access Denied', 'Cashiers cannot delete categories', 'warning');
        return;
      }
      Swal.fire({
        title: 'Are you sure?',
      ...
      $.get(api + 'users/all', function (users) {
        ...
        users.forEach((user, index) => {
          ...
          user_list += `<tr>
            <td>${user.fullname}</td>
            <td>${user.username}</td>
            <td class="${class_name}">${state.length > 0 ? state[0] : ''} <br><span style="font-size: 11px;"> ${
            state.length > 0 ? moment(state[1]).format('hh:mm A DD MMM YYYY') : ''
          }</span></td>
            <td>${
              user._id == 1
...
function loadSoldProducts() {
  ...
  sold.forEach((item, index) => {
    ...
    let institute = [];
    if (typeof allInstitutes !== 'undefined' && allInstitutes && allInstitutes.length > 0) {
...
function viewTransaction(index) {
  ...
  let refNumber =
    allTransactions[index].ref_number != '' ? allTransactions[index].ref_number : allTransactions[index].order;
  ...
  receipt = `<div style="max-width: 800px; margin: 0 auto; padding: 40px; font-family: Arial, sans-serif; background: #fff;">                            
        ...
        <div style="padding: 30px; border-radius: 8px; margin-bottom: 30px;">
            ...
            <div style="text-align: center; margin-bottom: 20px;">
                <h2 style="margin: 0; font-size: 24px; color: #333; font-weight: 700;">${settings.store}</h2>
                <p style="margin: 5px 0 0 0; font-size: 12px; color: #666;">${settings.address_one}</p>
                <p style="margin: 0; font-size: 12px; color: #666;">${settings.address_two}</p>
                <p style="margin: 0; font-size: 12px; color: #666;">${
                  settings.contact ? 'Tel: ' + settings.contact : ''
        ...
        <div style="display: flex; justify-content: space-between; margin-bottom: 30px;">
            ...
            <div style="text-align: right;">
                ...
                <div style="margin-bottom: 15px;">
                    <p style="margin: 0; font-weight: 700; font-size: 12px; color: #000;">Date</p>
                    <p style="margin: 0; font-size: 14px; color: #333;">${moment(allTransactions[index].date).format(
                      'MM/DD/YY',
                    )}</p>
                </div>
                <div>
        ...
        <div style="display: flex; justify-content: space-between; background: linear-gradient(to right, #f9f3e8, #fde8b8); padding: 20px; border-radius: 8px;">
            <div style="flex: 1; padding-right: 20px;">
                ...
                <p style="margin: 5px 0 0 0; font-size: 11px; color: #888;">Cashier: ${allTransactions[index].user}</p>
    ...
    allTransactions[index].total,
                ...
                ${
                  settings.charge_tax
                    ? `<p style="margin: 5px 0 0 0; font-size: 11px; color: #666;">Tax (${settings.percentage}%): ${
                        settings.symbol
                      }${parseFloat(allTransactions[index].tax).toFixed(2)}</p>`
                    : ''
                }
            </div>
        </div>
...
function authenticate() {
  ...
  console.log('Login HTML inserted, #loading children:', $('#loading').children().length);
